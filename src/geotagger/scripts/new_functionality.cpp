...
void flushDirectoryContents(const std::string& imageDirectory, const std::string& destinationBaseDir) {
    ...
    if (!fs::exists(imageDirectory) || !fs::is_directory(imageDirectory)) {
        std::cerr << RED_TEXT <<"Source directory does not exist or is not a directory." << std::endl;
        return;
    }
    ...
    if (fs::is_empty(imageDirectory)) {
        std::cout << RED_TEXT <<"Source directory is empty. No contents to flush." << std::endl;
        return;
    }
...
// void initializeCSVFile(const std::string& csvFileName, const std::string& csvFilePath) {
//     // Set the full path for the CSV file
//     std::string csvFilePathAndName = csvFilePath + csvFileName;

//     // Create a new CSV file with the provided name and path
//     std::ofstream csvFile(csvFilePathAndName, std::ios::trunc);
...
//         std::cout << "CSV file initialized: " << csvFilePathAndName << std::endl;
//     } else {
//         std::cerr << "Unable to open CSV file for initialization." << std::endl;
//     }
// }
...
void associateImageWithPreviousGPS(ros::Time imageCaptureTimestamp,const std::string& foundFileName) {
    ...
    if (lastIndexBeforeImageCapture != -1) {
        ...
        std::cout << "$'" << foundFileName << " " 
                ...
                << mavrosDataVector[lastIndexBeforeImageCapture].compass_heading + 180 << " "
        ...
        if (csvFile.is_open()) {
            ...
            csvFile << foundFileName << ","
                    << std::fixed << std::setprecision(9) 
                     ...
                     << mavrosDataVector[lastIndexBeforeImageCapture].altitude << ","
                     << mavrosDataVector[lastIndexBeforeImageCapture].compass_heading + 180 << ","
                     << mavrosDataVector[lastIndexBeforeImageCapture].yaw + 180 << ","
